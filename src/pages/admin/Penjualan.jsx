...
export default function Penjualan() {
  ...
  const searchBarang = async () => {
    ...
    setError("");
    try {
      const res = await api.get(`/barang/toko?search=${encodeURIComponent(search)}`);
      setResults(res.data || []);
  ...
  const handleMouseLeave = () => {
    // Tidak reset highlight saat mouse leave
  };
                ...
                <Box sx={{ mt: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                    ...
                    >
                      <option value=""></option>
                      {ketBayarList.map((k) => (
                        <option key={k.id_ket} value={k.id_ket}>{k.uraian}</option>
                      ))}
                    </TextField>
